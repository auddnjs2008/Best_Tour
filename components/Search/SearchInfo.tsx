...
interface ISearchInfo {
    ...
    keyIndex: number;
    setRecentSearch: Dispatch<React.SetStateAction<boolean>>;
    recentSearch: boolean;
}

export interface ISearchesResponse {
    ok: boolean;
    searches: RecentSearch[];
}

const SearchInfo = ({ setInputFocus, keyIndex, recentSearch, setRecentSearch }: ISearchInfo) => {
            ...
            <ul onClick={onClick}>
                {data && data.length !== 0 ?
                    data.map((item: any, index) =>
                        <li id={String(index)} key={item.id} className={cls(keyIndex === index ? "bg-yellow-400 " : "", "text-base cursor-pointer border-t p-3 flex content-center items-center")}>{item.place_name}</li>)
                    :
                    recentSearch ?
                        searchesData?.searches.map((item, index) =>
                            <li id={String(index)} key={index} className={cls(keyIndex === index ? "bg-yellow-400" : "", "text-base cursor-pointer border-t p-3 flex content-center items-center")}>
                                {item.isMarker ?
                        ...
                        :
                        searchesData?.searches.filter(item => item.isMarker).map((item, index) =>
                            <li id={String(index)} key={index} className={cls(keyIndex === index ? "bg-yellow-400" : "", "text-base cursor-pointer border-t p-3 flex content-center items-center")}>
                                <div className="rounded-full w-7 h-7 flex justify-center items-center border-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="text-gray-400 h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
